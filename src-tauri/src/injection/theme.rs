...
pub fn get_theme(name: String) -> Result<String, String> {
  ...
  if let Some(css_file) = css_file {
    return fs::read_to_string(theme_file.join(css_file)).map_err(|e| format!("Error reading CSS file: {}", e));
  }
...
pub fn get_theme_names() -> Result<Vec<String>, String> {
  ...
  let theme_folders = fs::read_dir(&themes_dir).map_err(|e| format!("Error reading theme directory: {}", e))?;
